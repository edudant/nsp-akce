-- Invalid shared-code attempts must commit their counter. Raising an exception
-- would roll back the whole RPC statement and make the bcrypt endpoint unlimited.
...
-- Supabase default privileges granted DML privileges on newly-created views.
-- The views are staff/member-only; shared readers use the narrow RPCs.
revoke all on
